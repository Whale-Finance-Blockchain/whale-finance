...
const talismanWindow =
  typeof window !== "undefined"
    ? (window as TalismanWindow)
    : ({} as TalismanWindow);
...
export class TalismanConnector extends AbstractConnector {
  ...
  public async activate(): Promise<any> {
    ...
    let account;
    try {
      account = await (talismanWindow.talismanEth.send as Send)(
        "eth_requestAccounts"
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId;
    try {
      chainId = await (talismanWindow.talismanEth.send as Send)(
        "eth_chainId"
    ...
    if (!chainId) {
      if ((talismanWindow.talismanEth as any).isDapper) {
        chainId = parseSendReturn(
          (talismanWindow.talismanEth as any).cachedResults.net_version
        );
      } else {
        chainId =
  ...
  public async getAccount(): Promise<null | string> {
    ...
    if (!account) {
      account = parseSendReturn(
        (talismanWindow.talismanEth.send as SendOld)({ method: "eth_accounts" })
      )[0];
    }
  ...
  public deactivate() {
    if (
      talismanWindow.talismanEth &&
      talismanWindow.talismanEth.removeListener
    ) {
      talismanWindow.talismanEth.removeListener(
        "chainChanged",
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (talismanWindow.talismanEth.send as Send)(
        "eth_accounts"
      ).then((sendReturn) => {
